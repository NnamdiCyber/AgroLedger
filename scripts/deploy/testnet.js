...
function writeEnv(admin, usdcContractId) {
  ...
  const content = `# ── Stellar Network (Testnet) ───────────────────────────────────────────
...
async function main() {
  console.log("=== AgroLedger Testnet Deploy ===\n");
  ...
  if (!deployerSecret || deployerSecret.startsWith("S...")) {
    console.log("A funded Stellar testnet secret key is required.");
    console.log(
  ...
  if (!usdcContractId) {
    console.log(
      "USDC contract ID is required (testnet: CCW67TSZV3SSS2HXMBQ5JFGCKJNXKZM7UQUWUZPUTHXSTZLEO7SJMI75)."
    );
    process.exit(1);
  }
  ...
  console.log("\n=== Deploy complete! ===");
  console.log("Run `npm run seed:local` (update .env for network first).");
  rl.close();
}
